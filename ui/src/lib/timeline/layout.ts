...
function kickoffEdgeRunScoreMs(edge: WorkTimelineEdge, span: WorkTimelineSpan): number {
  const start = spanStartMs(span);
  const at = new Date(edge.at).getTime();
  return at <= start ? start - at : (at - start) + 1e12;
}

function isClosestRunForKickoffEdge(edge: WorkTimelineEdge, span: WorkTimelineSpan, spans: readonly WorkTimelineSpan[]) {
  const currentDistance = kickoffEdgeRunScoreMs(edge, span);
  for (const other of spans) {
    if (other.runId === span.runId) continue;
    if (other.actorId !== edge.toActorId || other.issueId !== edge.issueId) continue;
    const otherDistance = kickoffEdgeRunScoreMs(edge, other);
    if (

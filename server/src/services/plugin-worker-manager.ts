 ...
 * minutes; deployments whose sandbox executions legitimately need longer (an
 * autonomous agent build runs as a single long `environmentExecute` RPC into
 * the sandbox, and the cap kills it — exit 137 — before it can persist the
 * workspace or record token usage) can raise it via
 * PAPERCLIP_PLUGIN_RPC_MAX_TIMEOUT_MS (milliseconds). Invalid or non-positive
 * values fall back to the default, so behavior is unchanged unless the
 * variable is set to a valid override.
 */
export function resolveMaxRpcTimeoutMs(
  env: Record<string, string | undefined> = process.env,
): number {
  return resolveEnvTimeoutMs(
    "PAPERCLIP_PLUGIN_RPC_MAX_TIMEOUT_MS",
    DEFAULT_MAX_RPC_TIMEOUT_MS,
    env,
  );
}
